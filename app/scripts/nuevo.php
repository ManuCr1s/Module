...
$lista = mysqli_query($link,"SELECT id_reclamo ,fecha ,detalles,estado, nombre_usuario,numero_documento,nombre, nombre_area FROM usuario 
INNER JOIN reclamo ON  usuario.numero_documento = reclamo.id_usuario 
INNER JOIN unidad ON reclamo.id_unidad = unidad.id
LEFT JOIN area ON area.id = reclamo.id_area WHERE estado = 'danger'");
